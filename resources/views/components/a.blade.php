...
<a {{ $disabledItem }} {{ $attributes->merge(['class' => "inline-flex justify-center items-center px-4 py-2 $disabledClasses $selectedColor border border-transparent rounded-md font-semibold text-xs text-white uppercase focus:outline-none focus:ring-2  focus:ring-offset-2 transition ease-in-out duration-150"]) }}>{{ $slot }}</a>
